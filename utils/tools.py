...
def vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            pred_np_2d = pred_np.reshape(-1, pred_np.shape[-1])
            true_np_2d = true_np.reshape(-1, true_np.shape[-1])

            # Add padding to match scaler feature dimensions
            pred_padded = np.zeros((pred_np_2d.shape[0], 5))  # Assuming 5 features
            pred_padded[:, -1] = pred_np_2d[:, 0]  # Place target in the last column

            # Inverse transform
            pred_padded_inverse = vali_data.scaler.inverse_transform(pred_padded)
            pred_np_2d = pred_padded_inverse[:, -1].reshape(-1, 1)

            # Same for true values
            true_padded = np.zeros((true_np_2d.shape[0], 5))
            true_padded[:, -1] = true_np_2d[:, 0]
            true_padded_inverse = vali_data.scaler.inverse_transform(true_padded)
            true_np_2d = true_padded_inverse[:, -1].reshape(-1, 1)
            
            mape_ = np.mean(np.abs((true_np_2d - pred_np_2d) / true_np_2d)) *100

            total_loss.append(loss.item())
            total_mae_loss.append(mae_loss.item())
            total_mape_loss.append(mape_)
